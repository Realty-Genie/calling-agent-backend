...
export class CallController {
  static async initiateCall(req: Request, res: Response) {
    try {
      ...
      if (!agentDoc && process.env.AGENT_ID) {
        try {
          agentDoc = await agentModel.findOne({
            retellAgentId: process.env.AGENT_ID,
          });
        } catch (e) {
      ...
      if (!agentDoc) {
        return res.status(404).json({
          success: false,
          message: "Agent not found or not configured",
        });
      }

      const userHasAccess = user.agents.some(
        (id: any) => id.toString() === agentDoc._id.toString(),
      );
      if (!userHasAccess) {
        return res
          .status(403)
          .json({ success: false, message: "Access denied to this agent" });
      }
      ...
      if (!fromNumber) {
        return res.status(404).json({
          success: false,
          message: "Agent phone number not found or not configured",
        });
      }
      ...
      try {
        ...
        return res.status(500).json({
          success: false,
          message: "Failed to initiate call with Retell",
        });
      }
  ...
  static async getCallDetails(req: Request, res: Response) {
    try {
      ...
      try {
        ...
        return res.status(500).json({
          success: false,
          message: "Failed to fetch call details from Retell",
        });
      }
      ...
      return res
        .status(500)
        .json({ success: false, message: "Failed to fetch call details" });
    }
  }
    ...
    try {
      ...
      for (const leadData of leads) {
        ...
        if (isSellerAgent && !address) {
          ...
          console.warn(
            `Skipping lead ${phoneNumber} because address is missing for seller agent.`,
          );
          continue;
        }
      ...
      try {
        ...
        return res.status(500).json({
          success: false,
          message: "Failed to save leads in batch call",
        });
      }
      ...
      if (tasks.length === 0) {
        return res.status(400).json({
          success: false,
          message:
            "No valid leads provided (check if address is present for seller agents)",
        });
      }
  ...
  static async publishCall(req: Request, res: Response) {
    const { scheduleType, metadata = {}, fromNumber, name, phNo } = req.body;
    const jobData = {
      scheduleType,
      metadata,
      fromNumber,
    ...
    await publishMessage(
      process.env.PUBSUB_TOPIC_NAME || "callgenie-topic",
      JSON.stringify({
        scheduleType,
